...
const DetailUserComponent = ({ visible, setVisible, setIsUpdateSuccess, productDetail }) => {
  ...
  const onFinish = async (newProduct) => {
    if(!thumbnail) {
      return setErrorThumbnail("Vui lòng thêm ảnh")
    }else{
      setErrorThumbnail('')
    }
    try {
      let res = await updateProductAPI(productDetail._id,{...newProduct, ...{thumbnail}});
      if (res.status) {
    ...
    <>
      <Modal title="Tạo mới" visible={visible} onCancel={handleCancel} footer={null} getContainer={false} id="modal-detail-account">
        <Form form={form} name="control-hooks" onFinish={onFinish}>
          <Typography.Paragraph><span style={{ color: 'red' }}>*</span>Nhập ảnh: </Typography.Paragraph>
          <Row>
            <Col span="5">
              <Image
                width={50}
            ...
            <Col span="19">
              <UploadImage getLinkImage={setThumbnail} />
            </Col>
          </Row>
          <span style={{ color: 'red' }}>{errorThumbnail}</span>
